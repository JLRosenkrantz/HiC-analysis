...
print("Running pipeline on: " + ", ".join(samples))
...
rule all:
    input:
        ...
        expand("data/pairix/{merged_prefix}.pairs.gz", merged_prefix=merged_prefix),
        expand("data/cool/{merged_prefix}.cool", merged_prefix=merged_prefix),
        expand("data/cool/{merged_prefix}.mcool", merged_prefix=merged_prefix),
        expand("data/TADs/{merged_prefix}_min10_max60_fdr01_d01_boundaries.bed", merged_prefix=merged_prefix),
#         expand("data/TADs/{merged_prefix}_min30_max100_fdr01_d001_boundaries.bed", merged_prefix=merged_prefix),
...
rule cooler_cload:
    ...
    conda:
        "envs/hic.yml"
    log:
        "data/logs/cooler_cload.{merged_prefix}.log"  # Remove expand and use the same wildcard
    threads: 16
    shell:
